...
class TestUpdateWindowsQualityUpdates(unittest.TestCase):
    """Test class for update_windowsQualityUpdates."""

    def setUp(self):
        self.directory = TempDirectory()
        self.directory.create()
        self.directory.makedir("Quality Updates")
        self.directory.write(
            "Quality Updates/test.json",
            '{"test": "test"}',
            encoding="utf-8",
        )
        self.token = "token"
            ...
            "value": [
                {
                    ...
                    "displayName": "test",
                    "testvalue": "test",
                    "expeditedUpdateSettings": {"test": "test"},
        ...
        self.batch_assignment_patch = patch(
            "src.IntuneCD.update.Intune.update_windowsQualityUpdates.batch_assignment"
        )
        self.batch_assignment = self.batch_assignment_patch.start()

        self.object_assignment_patch = patch(
            "src.IntuneCD.update.Intune.update_windowsQualityUpdates.get_object_assignment"
        )
        self.object_assignment = self.object_assignment_patch.start()

        self.makeapirequest_patch = patch(
            "src.IntuneCD.update.Intune.update_windowsQualityUpdates.makeapirequest"
        )
        self.makeapirequest = self.makeapirequest_patch.start()
        self.makeapirequest.return_value = self.mem_data

        self.update_assignment_patch = patch(
            "src.IntuneCD.update.Intune.update_windowsQualityUpdates.update_assignment"
        )
        self.update_assignment = self.update_assignment_patch.start()
        self.update_assignment.return_value = [{"target": {"groupName": "test"}}]

        self.load_file_patch = patch(
            "src.IntuneCD.update.Intune.update_windowsQualityUpdates.load_file"
        )
        self.load_file = self.load_file_patch.start()
        self.load_file.return_value = self.repo_data

        self.post_assignment_update_patch = patch(
            "src.IntuneCD.update.Intune.update_windowsQualityUpdates.post_assignment_update"
        )
        self.post_assignment_update = self.post_assignment_update_patch.start()

        self.makeapirequestPatch_patch = patch(
            "src.IntuneCD.update.Intune.update_windowsQualityUpdates.makeapirequestPatch"
        )
        self.makeapirequestPatch = self.makeapirequestPatch_patch.start()

        self.makeapirequestPost_patch = patch(
            "src.IntuneCD.update.Intune.update_windowsQualityUpdates.makeapirequestPost"
        )
        self.makeapirequestPost = self.makeapirequestPost_patch.start()
        self.makeapirequestPost.return_value = {"id": "0"}

        self.makeapirequestDelete_patch = patch(
            "src.IntuneCD.update.Intune.update_windowsQualityUpdates.makeapirequestDelete"
        )
