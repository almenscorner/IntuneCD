...
class TestUpdateEnrollmentStatusPage(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.batch_assignment_patch = patch(
            "src.IntuneCD.update.Intune.update_enrollmentStatusPage.batch_assignment"
        )
        self.batch_assignment = self.batch_assignment_patch.start()

        self.object_assignment_patch = patch(
            "src.IntuneCD.update.Intune.update_enrollmentStatusPage.get_object_assignment"
        )
        self.object_assignment = self.object_assignment_patch.start()

        self.makeapirequest_patch = patch(
            "src.IntuneCD.update.Intune.update_enrollmentStatusPage.makeapirequest"
        )
        self.makeapirequest = self.makeapirequest_patch.start()
        self.makeapirequest.side_effect = self.mem_data, self.app_ids

        self.update_assignment_patch = patch(
            "src.IntuneCD.update.Intune.update_enrollmentStatusPage.update_assignment"
        )
        self.update_assignment = self.update_assignment_patch.start()

        self.load_file_patch = patch(
            "src.IntuneCD.update.Intune.update_enrollmentStatusPage.load_file"
        )
        self.load_file = self.load_file_patch.start()
        self.load_file.return_value = self.repo_data

        self.post_assignment_update_patch = patch(
            "src.IntuneCD.update.Intune.update_enrollmentStatusPage.post_assignment_update"
        )
        self.post_assignment_update = self.post_assignment_update_patch.start()

        self.makeapirequestPatch_patch = patch(
            "src.IntuneCD.update.Intune.update_enrollmentStatusPage.makeapirequestPatch"
        )
        self.makeapirequestPatch = self.makeapirequestPatch_patch.start()

        self.makeapirequestPost_patch = patch(
            "src.IntuneCD.update.Intune.update_enrollmentStatusPage.makeapirequestPost"
        )
        self.makeapirequestPost = self.makeapirequestPost_patch.start()
        self.makeapirequestPost.return_value = {"id": "0"}

        self.makeapirequestDelete_patch = patch(
            "src.IntuneCD.update.Intune.update_enrollmentStatusPage.makeapirequestDelete"
        )
    ...
    def test_remove_config(self):
        """makeapirequestDelete should be called once."""
