...
class TestBackupProactiveRemediation(unittest.TestCase):
    ...
    def test_publisher_is_microsoft(self):
        ...
        with patch("src.IntuneCD.backup_proactiveRemediation.batch_request", side_effect=[[{"publisher": "Microsoft"}]]):
            self.count = savebackup(self.directory.path, "json", self.exclude, self.token)
    ...
    def test_backup_with_no_returned_data(self):
        ...
        with patch("src.IntuneCD.backup_proactiveRemediation.makeapirequest", return_value={"value": []}):
            self.count = savebackup(self.directory.path, "json", self.exclude, self.token)
