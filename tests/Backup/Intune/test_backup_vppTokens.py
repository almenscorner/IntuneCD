...
from src.IntuneCD.backup.Intune.backup_vppTokens import savebackup
...
class TestBackupVPPTokens(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patch_makeapirequest = patch(
            "src.IntuneCD.backup.Intune.backup_vppTokens.makeapirequest",
            return_value=self.vpp_token,
        )
        self.makeapirequest = self.patch_makeapirequest.start()

        self.makeAuditRequest_patch = patch(
            "src.IntuneCD.backup.Intune.backup_vppTokens.makeAuditRequest"
        )
        self.get_scope_tags = self.makeAuditRequest_patch.start()
        self.get_scope_tags.return_value = self.audit_data
