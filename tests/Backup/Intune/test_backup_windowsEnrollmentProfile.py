...
from src.IntuneCD.backup.Intune.backup_windowsEnrollmentProfile import savebackup
...
class TestBackupWindowsEnrollmentProfile(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.batch_assignment_patch = patch(
            "src.IntuneCD.backup.Intune.backup_windowsEnrollmentProfile.batch_assignment"
        )
        self.batch_assignment = self.batch_assignment_patch.start()
        self.batch_assignment.return_value = BATCH_ASSIGNMENT

        self.object_assignment_patch = patch(
            "src.IntuneCD.backup.Intune.backup_windowsEnrollmentProfile.get_object_assignment"
        )
        self.object_assignment = self.object_assignment_patch.start()
        self.object_assignment.return_value = OBJECT_ASSIGNMENT

        self.makeapirequest_patch = patch(
            "src.IntuneCD.backup.Intune.backup_windowsEnrollmentProfile.makeapirequest"
        )
        self.makeapirequest = self.makeapirequest_patch.start()
        self.makeapirequest.return_value = self.enrollment_profile

        self.makeAuditRequest_patch = patch(
            "src.IntuneCD.backup.Intune.backup_windowsEnrollmentProfile.makeAuditRequest"
        )
    ...
    def test_backup_append_id(self):
        ...
        self.assertTrue(
            Path(
                f"{self.directory.path}/Enrollment Profiles/Windows/test__0.json"
            ).exists()
        )

    def test_backup_scope_tags_and_aduit(self):
        """The folder should be created, the file should have the expected contents, and the count should be 1."""
