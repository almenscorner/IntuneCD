...
from src.IntuneCD.backup.Intune.backup_enrollmentStatusPage import savebackup
...
class TestBackupEnrollmentStatusPage(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.batch_assignment_patch = patch(
            "src.IntuneCD.backup.Intune.backup_enrollmentStatusPage.batch_assignment"
        )
        self.batch_assignment = self.batch_assignment_patch.start()
        self.batch_assignment.return_value = BATCH_ASSIGNMENT

        self.object_assignment_patch = patch(
            "src.IntuneCD.backup.Intune.backup_enrollmentStatusPage.get_object_assignment"
        )
        self.object_assignment = self.object_assignment_patch.start()
        self.object_assignment.return_value = OBJECT_ASSIGNMENT

        self.makeapirequest_patch = patch(
            "src.IntuneCD.backup.Intune.backup_enrollmentStatusPage.makeapirequest"
        )
    ...
    def test_backup_append_id(self):
        ...
        self.count = savebackup(
            self.directory.path, "json", self.exclude, self.token, "", True, None
        )
    ...
    def test_backup_scope_tags(self):
        """The folder should be created, the file should have the expected contents, and the count should be 1."""
