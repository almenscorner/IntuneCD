...
from src.IntuneCD.backup.Intune.backup_shellScripts import savebackup

BATCH_ASSIGNMENT = [{"value": [{"id": "0", "target": {"groupName": "Group1"}}]}]
OBJECT_ASSIGNMENT = [{"target": {"groupName": "Group1"}}]


class TestBackupShellScript(unittest.TestCase):
    """Test class for backup_shellScripts."""
    ...
    def setUp(self):
        ...
        self.script_content_path = (
            f"{self.directory.path}/Scripts/Shell/Script Data/test.sh"
        )
        self.expected_data = {
        ...
        self.object_assignment_patch = patch(
            "src.IntuneCD.backup.Intune.backup_shellScripts.get_object_assignment"
        )
        self.object_assignment = self.object_assignment_patch.start()
        self.object_assignment.return_value = OBJECT_ASSIGNMENT

        self.batch_request_patch = patch(
            "src.IntuneCD.backup.Intune.backup_shellScripts.batch_request"
        )
        self.batch_request = self.batch_request_patch.start()
        self.batch_request.return_value = self.batch_request_data

        self.makeapirequest_patch = patch(
            "src.IntuneCD.backup.Intune.backup_shellScripts.makeapirequest"
        )
