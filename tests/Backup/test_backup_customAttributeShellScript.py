...
class TestBackupCustomAttributeShellScript(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.batch_assignment_patch = patch(
            "src.IntuneCD.backup.backup_customAttributeShellScript.batch_assignment"
        )
        self.batch_assignment = self.batch_assignment_patch.start()
        self.batch_assignment.return_value = BATCH_ASSIGNMENT

        self.object_assignment_patch = patch(
            "src.IntuneCD.backup.backup_customAttributeShellScript.get_object_assignment"
        )
        self.object_assignment = self.object_assignment_patch.start()
        self.object_assignment.return_value = OBJECT_ASSIGNMENT

        self.batch_request_patch = patch(
            "src.IntuneCD.backup.backup_customAttributeShellScript.batch_request"
        )
        self.batch_request = self.batch_request_patch.start()
        self.batch_request.return_value = self.batch_request_data

        self.makeapirequest_patch = patch(
            "src.IntuneCD.backup.backup_customAttributeShellScript.makeapirequest"
        )
