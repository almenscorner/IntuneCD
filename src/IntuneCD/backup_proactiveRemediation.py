        ...
        if "Microsoft" not in pr['publisher']:
            ...
            if output != 'json':
                with open(f"{configpath}{fname}.yaml",'w') as yamlFile:
                    yaml.dump(pr_details,yamlFile,sort_keys=False,default_flow_style=False)
            else:
            ...
            f = open(f"{configpath}/Script Data/{pr_details['displayName']}_DetectionScript.ps1",'w')
            f.write(decoded)
            ...
            f = open(f"{configpath}/Script Data/{pr_details['displayName']}_RemediationScript.ps1",'w')
            f.write(decoded)