...
def savebackup(path, output, token):
    ...
    for profile in data['value']:
        profile_data = makeapirequest(
            endpoint + profile['id'] + '/enrollmentProfiles', token)
        for p in profile_data['value']:
            p_data = makeapirequest(
                endpoint + profile['id'] + '/enrollmentProfiles/' + p['id'], token)
            pid = p_data['id']
            ...
            if output != "json":
                with open(configpath+fname+".yaml", 'w') as yamlFile:
                    yaml.dump(p_data, yamlFile, sort_keys=False,
