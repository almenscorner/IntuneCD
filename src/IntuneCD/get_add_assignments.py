...
def get_assignments(endpoint, get_object, objectID, token, extra_endpoint=None):
    remove_keys = {'id', 'createdDateTime',
                   'version', 'lastModifiedDateTime', 'sourceId'}
    current_assignments = []
    q_param = {"$select": "displayName"}
    if extra_endpoint == None:
        assignments = makeapirequest(
            endpoint + "/" + objectID + "/assignments", token)
    else:
        assignments = makeapirequest(
            extra_endpoint + "/" + objectID + "/assignments", token)
        ...
        for assignment in assignments['value']:
            ...
            if "groupId" in assignment['target']:
                group_name = makeapirequest(
                    group_endpoint + "/" + assignment['target']['groupId'], token, q_param)
                if group_name:
            ...
            if assignment['target']['deviceAndAppManagementAssignmentFilterId']:
                if assignment['target']['deviceAndAppManagementAssignmentFilterId'] != "00000000-0000-0000-0000-000000000000":
                    filter_name = makeapirequest(
                        filter_endpoint + "/" + assignment['target']['deviceAndAppManagementAssignmentFilterId'], token, q_param)
                    if filter_name:
...
def add_assignment(endpoint, add_object, objectID, token, status_code=200,
    ...
    if "assignments" in add_object:
        ...
        if extra_endpoint == None:
            curr_assignment = get_assignments(
                endpoint, add_object, objectID, token)
        else:
            curr_assignment = get_assignments(
                endpoint, add_object, objectID, token, extra_endpoint)
            ...
            if curr_assignment != repo_assignments:
                for assignment in repo_assignments:
                    ...
                    if "groupName" in assignment['target']:
                        ...
                        group_q_param = {
                            "$filter": "displayName eq " + "'" + assignment['target']['groupName'] + "'"}
                        group_data = makeapirequest(
                            group_endpoint, token, group_q_param)
                        if group_data['value']:
                ...
                if assign:
                    if new_assignments:
                        print("Updating assignment for: " +
                              objectID + " with assignment(s):")
                    ...
                    if ((extra_url == None) and (wap == False) and (script == False) and (proactive_remediation == False)):
                        ...
                        makeapirequestPost(endpoint + "/" + objectID + "/assign", token,
                                           q_param=None, jdata=request_json, status_code=status_code)
                    elif wap == True:
                        for target in assign:
                            request_data['target'] = target['target']
                            request_json = json.dumps(request_data)
                            makeapirequestPost(endpoint + "/" + objectID + "/assignments",
                                               token, q_param=None, jdata=request_json, status_code=status_code)
                    elif script == True:
                        request_data['deviceManagementScriptAssignments'] = assign
                        request_json = json.dumps(request_data)
                        makeapirequestPost(
                            endpoint + "/" + objectID + "/assign", token, q_param=None, jdata=request_json)
                        ...
                        request_data['assignments'] = assign
                        request_json = json.dumps(request_data)
                        makeapirequestPost(endpoint + "/" + objectID + extra_url + "/assign",
                                           token, q_param=None, jdata=request_json, status_code=status_code)
                ...
                if assignment['target']['deviceAndAppManagementAssignmentFilterId']:
                    ...
                    if filter_id is None:
                        ...
                        assignment['target'].pop(
                            'deviceAndAppManagementAssignmentFilterType')
                if "groupName" in assignment['target']:
                    group_q_param = {"$filter": "displayName eq " +
                                     "'" + assignment['target']['groupName'] + "'"}
                    group_data = makeapirequest(
                        group_endpoint, token, group_q_param)
                    if group_data['value']:
                        assignment['target'].pop('groupName')
