...
BASE_ENDPOINT = "https://graph.microsoft.com/beta/policies/authenticationFlowsPolicy"
...
def update(path, token, report):
    ...
    # If authentication flows policy path exists, continue
    if file:
        ...
        if entra_data:
            print("-" * 90)
            diff = DeepDiff(
                entra_data["selfServiceSignUp"],
                repo_data["selfServiceSignUp"],
                ignore_order=True,
            ).get("values_changed", {})
            ...
            if diff and report is False:
                request_data = json.dumps(
                    {"selfServiceSignUp": repo_data["selfServiceSignUp"]}
                )
                makeapirequestPatch(
                    BASE_ENDPOINT,
                    token,
