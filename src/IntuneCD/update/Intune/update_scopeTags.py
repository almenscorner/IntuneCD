...
"""
This module is used to update all Scope Tags in Intune.
"""
...
def update(
    ...
    """
    This function updates all Scope Tags in Intune,
    ...
    # If Scope Tags path exists, continue
    if os.path.exists(configpath):
        ...
        for filename in os.listdir(configpath):
            ...
            # If Scope Tag does not exist, create it and assign
            else:
                print("-" * 90)
                print("Scope Tag not found, creating tag: " + repo_data["displayName"])
                if report is False:
                    ...
                    print("Scope Tag created with id: " + post_request["id"])

        # If any Scope Tags are left in mem_data, remove them from Intune as they are not in the repo
        if mem_data.get("value", None) is not None and remove is True:
            for val in mem_data["value"]:
                if val["isBuiltIn"] is False:
                    print("-" * 90)
                    print("Removing Scope Tag from Intune: " + val["displayName"])
                    if report is False:
                        makeapirequestDelete(
