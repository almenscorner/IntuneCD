...
"""
This module is used to update all Windows Driver Update Profiles in Intune.
"""
...
from src.IntuneCD.intunecdlib.graph_batch import batch_assignment, get_object_assignment
...
def update(
    ...
    """
    This function updates all Windows Driver Update Profiles in Intune,
    ...
    diff_summary = []
    # Set Windows Driver Update Profile path
    configpath = path + "/" + "Driver Updates"
    # If Windows Driver Update Profile path exists, continue
    if os.path.exists(configpath):
        ...
        for filename in os.listdir(configpath):
            ...
            # If Windows Driver Update Profile exists, continue
            if data["value"]:
                print("-" * 90)
                mem_id = data.get("value").get("id")
                # Remove keys before using DeepDiff
                data["value"] = remove_keys(data["value"])
                data["value"].pop("newUpdates", None)
                data["value"].pop("inventorySyncStatus", None)
                data["value"].pop("deviceReporting", None)
                data["value"].pop("approvalType", None)
                ...
                if diff and report is False:
                    repo_data.pop("newUpdates", None)
                    repo_data.pop("inventorySyncStatus", None)
                    repo_data.pop("deviceReporting", None)
                    repo_data.pop("approvalType", None)
            ...
            # If Driver Update profile does not exist, create it and assign
            else:
                print("-" * 90)
                print(
                    "Driver Update profile not found, creating profile: "
                ...
                if report is False:
                    ...
                    print(
                        "Driver Update profile created with id: " + post_request["id"]
                    )

        # If any Windows Driver Update Profile are left in mem_data, remove them from Intune as they are not in the repo
        if mem_data.get("value", None) is not None and remove is True:
            for val in mem_data["value"]:
                print("-" * 90)
                print(
                    "Removing Driver Update Profile from Intune: " + val["displayName"]
                )
                if report is False:
                    makeapirequestDelete(
