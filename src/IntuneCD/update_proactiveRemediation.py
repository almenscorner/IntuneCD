            ...
            with open(file) as f:
                    ...
                    if mem_proactiveRemediation['value']:
                        ...
                        if os.path.exists(detection_script_name) and os.path.exists(remediation_script_name):
                            ...
                            cdiff = DeepDiff(mem_data, repo_data, ignore_order=True, exclude_paths=["root['detectionScriptContent']","root['remediationScriptContent']"]).get('values_changed',{})

                            ## If any changed values are found, push them to Intune
                            if cdiff or ddiff or rdiff:
                                print("Updating Proactive Remediation: " + repo_data['displayName'] + ", values changed:")
                                print(*cdiff.items(), sep='\n')
                                if ddiff:
                                    print("Detection script changed, check commit history for change details")
                                if rdiff:
                                    print("Remediation script changed, check commit history for change details")
                                ...
                                print('No difference found for Proactive Remediation: ' + repo_data['displayName'])
